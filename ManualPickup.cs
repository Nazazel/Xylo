...
public class ManualPickup : MonoBehaviour {
	...
	void Update()
	{
		if (player.GetComponent<PlayerController> ().activeTab == false && player.GetComponent<PlayerController> ().currentObjective == 5 && waitForPress && Input.GetKey (KeyCode.E) && !player.GetComponent<PlayerController> ().manual && !player.GetComponent<PlayerController> ().activeHint) {
