...
public class DeadCrewSight : MonoBehaviour {
	...
	public IEnumerator deathReaction ()
	{
		...
		if (triggerBox.name == "Medic") {
			...
			player.GetComponent<PlayerController> ().hintText.text = "Stella: ...she's dead.";
			player.GetComponent<PlayerController> ().numDead += 1;
		} else if (triggerBox.name == "Engineer") {
			player.GetComponent<PlayerController> ().playerAnimator.Play ("StellaFaceCover");
			player.GetComponent<PlayerController> ().hintText.text = "Stella: Dear Lord...he's...dead.";
			player.GetComponent<PlayerController> ().numDead += 1;
		} else if (triggerBox.name == "Red Shirt") {
			player.GetComponent<PlayerController> ().playerAnimator.Play ("StellaFaceCover");
			player.GetComponent<PlayerController> ().hintText.text = "Stella: H-he's dead.";
			player.GetComponent<PlayerController> ().numDead += 1;
		} else if (triggerBox.name == "Navigation") {
			player.GetComponent<PlayerController> ().playerAnimator.Play ("StellaFaceCover");
			player.GetComponent<PlayerController> ().hintText.text = "Stella: Jesus Christ, he's...";
			player.GetComponent<PlayerController> ().numDead += 1;
		} else if (triggerBox.name == "Captain") {
			player.GetComponent<PlayerController> ().playerAnimator.Play ("StellaFaceCover");
			player.GetComponent<PlayerController> ().hintText.text = "Stella: My God, no..t-the captain's dead.";
			player.GetComponent<PlayerController> ().numDead += 1;
		}
		...
		if (player.GetComponent<PlayerController> ().numDead == 5) {
			...
			player.GetComponent<PlayerController> ().hintText.text = "Stella: What do I do? What do I do?! I'm alone!I need help...I need help!\n\tI...I need to send out a signal.\n\tI have to go to the communications room and send out an SOS signal! ";
			yield return new WaitUntil (() => Input.GetKeyDown (KeyCode.Return));
			yield return new WaitForSeconds (0.2f);
		}
