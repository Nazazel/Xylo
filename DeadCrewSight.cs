...
public class DeadCrewSight : MonoBehaviour {
	...
	{
		if (player.GetComponent<PlayerController> ().activeHint == true) {
			yield return new WaitUntil (() => player.GetComponent<PlayerController> ().activeHint == false);
		}
		else if (player.GetComponent<PlayerController> ().activeTab == true) {
			yield return new WaitUntil (() => player.GetComponent<PlayerController> ().activeTab == false);
		}
		...
		if (triggerBox.name == "Medic") {
			...
			player.GetComponent<PlayerController> ().playerAnimator.Play ("StellaFaceCover");
			player.GetComponent<PlayerController> ().hintText.text = "<color=fuchsia>Stella</color>: He's...gone.";
			player.GetComponent<PlayerController> ().numDead += 1;
		} else if (triggerBox.name == "Navigation") {
			player.GetComponent<PlayerController> ().playerAnimator.Play ("StellaFaceCover");
			player.GetComponent<PlayerController> ().hintText.text = "<color=fuchsia>Stella</color>: No...he's dead";
			player.GetComponent<PlayerController> ().numDead += 1;
		} else if (triggerBox.name == "Captain") {
			player.GetComponent<PlayerController> ().playerAnimator.Play ("StellaFaceCover");
			player.GetComponent<PlayerController> ().hintText.text = "<color=fuchsia>Stella</color>:  My God...the captain's dead.";
			player.GetComponent<PlayerController> ().numDead += 1;
		}
		...
		if (player.GetComponent<PlayerController> ().numDead == 5) {
			...
			yield return new WaitUntil (() => Input.GetKeyDown (KeyCode.Return));
			yield return new WaitForSeconds (0.2f);
			player.GetComponent<PlayerController> ().hintText.text = "<color=fuchsia>Stella</color>: I...I need to send out a signal.\n\tI have to go to the communications room and send out an SOS signal! ";
			yield return new WaitUntil (() => Input.GetKeyDown (KeyCode.Return));
			yield return new WaitForSeconds (0.2f);
		}
