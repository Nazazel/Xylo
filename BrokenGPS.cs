...
public class BrokenGPS : MonoBehaviour {
	...
	void Update()
	{
		if (player.GetComponent<PlayerController> ().activeTab == false && player.GetComponent<PlayerController> ().activeHint == false && player.GetComponent<PlayerController> ().finishedJump == true && startGPSDialogue == false && Input.GetKey (KeyCode.E) && waitForPress) {
